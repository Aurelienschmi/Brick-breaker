...
void displayTitleLoose(SDL_Renderer *renderer, char* path){
    TTF_Font* font = TTF_OpenFont (path, 200);
    SDL_Color color = {0,102,204};
    SDL_Surface* surface = TTF_RenderText_Blended(font, "You Loose", color);
...
void displaySubTitleLoose(SDL_Renderer *renderer, char* path, int r, int g, int b){
    TTF_Font* font = TTF_OpenFont (path, 200);
...
void menuLoose(SDL_Renderer* renderer){
    ...
    while (!quit) {
        while (SDL_PollEvent(&event)) {
            ...
            displayTitleLoose(renderer, "../fonts/findet-nemo/Findet Nemo.ttf");
            displaySubTitleLoose(renderer, "../fonts/findet-nemo/Findet Nemo.ttf", r, g, b);
            SDL_RenderPresent(renderer);
        }
    }
}