//
// Created by schmi on 14/01/2024.
//
...
void displaySubTitleEasterEgg(SDL_Renderer *renderer, char* path, int r, int g, int b){
...
void menuEasterEgg(SDL_Renderer* renderer){
    ...
    SDL_Surface* image = IMG_Load("../img/background_menu_easterEgg.jpg");
    ...
    while (!quit) {
        while (SDL_PollEvent(&event)) {
            ...
            if (easterEgg){
                startEasterEgg(renderer);
                quit = true;
            }
            // Afficher le sous-titre avec la couleur mise à jour
            displaySubTitleEasterEgg(renderer, "../fonts/findet-nemo/Findet Nemo.ttf", r, g, b);
            SDL_RenderPresent(renderer);
        }
    }
}